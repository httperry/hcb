...
class AdminController < Admin::BaseController
  ...
  def transaction
    ...
    @mapping_history = @canonical_transaction.mapping_history
    # Mapping to a wire or Wise transfer changes the transaction's HCB code, so when it has carried
    # more than one, show which each mapping was made under.
    @show_mapping_hcb_codes = @mapping_history.filter_map { |m| m[:hcb_code] }.uniq.many?
    ...
    if first_mapping && first_mapping[:time] < Time.current.beginning_of_month
      ...
      first_mapped_event = first_mapping[:event]
  ...
  def set_event
    ...
    mapping_history = canonical_transaction.mapping_history
    first_mapping = mapping_history.first
    stale_remap = first_mapping && first_mapping[:time] < Time.current.beginning_of_month
    ...
    if stale_remap
      AdminMailer.with(
        ...
        user: current_user,
        previous_event:,
        first_mapped_at: first_mapping[:time],
        # The history as it was, plus the mapping just made - cheaper than reading it back.
        mapping_history: mapping_history + [{
          time: Time.current,
          user: current_user,
          event: @canonical_transaction.event,
          event_id: @canonical_transaction.event&.id,
          automatic: false,
          hcb_code: @canonical_transaction.local_hcb_code
        }]
